...
function shouldServeApiBrowserView(req) {
  if (req.method !== 'GET') return false;
  if (String(req.query.raw || '').trim() === '1') return false;
  const secFetchDest = normalizeString(req.get('sec-fetch-dest')).toLowerCase();
  const secFetchMode = normalizeString(req.get('sec-fetch-mode')).toLowerCase();
  return secFetchDest === 'document' || secFetchMode === 'navigate';
}

function renderApiBrowserPage(req) {
  const currentUrl = new URL(req.originalUrl, `${req.protocol}://${req.get('host')}`);
  const pageTitle = `Burchalkin Cup — API ${req.path}`;
...
<body>
  <main>
    <section class="card">
      ...
      <h1>${escapeHtml(req.path)}</h1>
      <p>Этот URL открыт в браузере, поэтому данные показаны в удобном виде. Для сайта и админки API продолжает работать как обычный JSON.</p>
      <pre id="api-json">Загрузка...</pre>
    </section>
  </main>
  <script>
    (async function () {
      const output = document.getElementById('api-json');
      try {
        const url = new URL(${JSON.stringify(currentUrl.toString())});
...
app.use('/api', (req, res, next) => {
  if (!shouldServeApiBrowserView(req)) {
    return next();
  }

  return res.type('html').send(renderApiBrowserPage(req));
});
